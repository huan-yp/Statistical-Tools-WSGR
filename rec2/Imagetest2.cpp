...
namespace NoiseReduction {
	...
	void Merge(ImageMatri& Target, string Type) {
		...
		if (cnt) {
			flag = 0;
			printf("Type:");
			if (Type == "MISS")StatuCount[3]++,cout<<Type<<",Amount:"<< StatuCount[3]<<endl;
			if (Type == "YELLOW")StatuCount[2]++, cout << Type << ",Amount:" << StatuCount[2] << endl;
			if (Type == "NORMAL")StatuCount[1]++, cout << Type << ",Amount:" << StatuCount[1] << endl;
		}
		//for (int k = 1;k <= 6;k++) {
		//	if (KeyPointCount[k] >= 100)
...
void ProcessImage(ImageMatri &Img) {
	...
	if (flag==0)Img.SaveImage(NumberToString(FrameNow)+"Get.PNG");
}
...
int main() {
	...
	*/
	{
		MathMat[1].Set(555, 715, 1142, 1382);
		MathMat[2].Set(499, 665, 1396, 1590);
		MathMat[3].Set(377, 544, 1137, 1354);
		MathMat[4].Set(307, 473, 1341, 1562);
		MathMat[5].Set(227, 365, 983, 1235);
		MathMat[6].Set(139, 307, 1221, 1465);
	}
	...
	while (1) {
		...
		capture >> img;
		if (lst - FrameNow >= FightGap * VideoRate) {
